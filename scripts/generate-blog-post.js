...
async function generatePost(targetDateStr = null) {
  ...
  if (minCount >= 1) {
    ...
    console.log("애드센스 승인 통과 전까지 추가 중복 콘텐츠 생성을 차단하기 위해 글 자동 생성을 일시 대기합니다.");
    console.log("--------------------------------------------------------------------------------");
...
대원칙: 이전에 작성된 블로그 내용과 절대 중복되는 문장이나 내용을 재사용하지 마라. 애드센스 승인과 상위 노출을 위해 '광범위한 백과사전식 글'을 절대 피하고, 반드시 '특정 상황과 특정 증상'에 맞춘 롱테일(Long-tail) 타겟팅 글로 작성해야 한다.`;
...
1. 서론 (Introduction):
   - 일반적인 원인 설명이 아니라, '사무실 의자에서 일어날 때', '아침에 침대에서 일어날 때'와 같이 독자가 일상에서 겪는 **매우 구체적인 특정 통증 상황**을 설정하고 이에 깊이 공감하는 문장으로 시작.
   - 가벼운 통증이라고 방치했을 때 찾아오는 만성 질환으로의 발전 위험성을 경고하며,지금 관리를 시작해야 하는 이유(본문의 필요성)를 강력히 강조.
   - 재활에 대한 최신 트렌드와 롱테일 검색어를 자연스럽게 활용하여 도입부를 매력적으로 작성.

2. 본론 1 (Causes & Symptoms):
   - 설정한 특정 상황에서 질환이 발생하는 원인과 발달 과정을 해부학적·의학적 사실에 기반하여 일반인의 눈높이에 맞춰 명확하게 설명.
...
- **롱테일 키워드 (Long-tail SEO) 적용 필수**: 제목과 본문은 절대로 '허리디스크' 처럼 광범위한 단어로 구성해서는 안 된다. "50대 여성이 자고 일어났을 때 발뒤꿈치가 찢어질 듯 아픈 이유와 아침 3분 스트레칭" 처럼 대상, 상황, 증상을 좁혀 구체적으로 설정할 것.
...
- **후킹 타이틀**: 블로그 제목을 선정할 때는 클릭을 유도하는 강력한 후킹 기법을 적용하되, 반드시 **초구체적(Long-tail)인 상황 묘사**를 포함할 것. (예시: "사무실 의자에서 일어날 때 오른쪽 허리 찌릿? 방치하면 디스크 터집니다")`;
...
FILENAME: ${todayStr}-keyword`;
  ...
  // 4. Gemini API 호출 (과부하 대비 자동 폴백 모델들 & 3회 자동 재시도 로직 탑재)
  ...
  for (const modelName of candidateModels) {
    console.log(`Gemini API 호출 시도 중... (모델: ${modelName})`);
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      try {
        ...
        console.warn(`모델 ${modelName} - 시도 ${attempt} 실패 (상태 코드: ${response.status}). 응답: ${errText}`);
      } catch (err) {
        console.warn(`모델 ${modelName} - 시도 ${attempt} 연결 실패: ${err.message}`);
      }
    ...
    if (success) {
      console.log(`성공적으로 블로그 본문을 생성했습니다. (사용된 모델: ${finalModelUsed})`);
      break;
    } else {
      console.warn(`모델 ${modelName} 호출 실패. 다음 후보 모델로 넘어갑니다.`);
    }
  }

  try {
    if (!success || !response) {
      throw new Error(`Gemini API 호출 실패 (모든 후보 모델 시도 실패)`);
    }
